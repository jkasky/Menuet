...
2. **Menu indexing (`AXMenu.swift`, `Menu.swift`, `FuzzyMatch.swift`)** — `AXMenuWalker` traverses `MenuBar → MenuBarItem → Menu → MenuItem` recursively. `AXMenuIndexer` (an `AXMenuVisitor`) extracts title/shortcut/enabled state per leaf and appends to `MenuIndex`'s flat `[MenuItem]`. On `find(query:)`, `MenuIndex` runs `FuzzyMatch.score` (an fts_fuzzy_match-style scorer) against each item's title and returns results sorted by score — prefix, word-boundary, camelCase, and consecutive-run matches rank higher. `MenuItemCommand.perform()` dispatches via an `AXMenuItemDelegate` that does an `AXPress`, with a path-based fallback (`AXMenuItemPath`) when the captured element is no longer valid.
